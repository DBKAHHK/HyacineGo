...
import (
	...
	"hyacine-server/internal/gameserver/account"
...
func (db *DataBase) savePlayerLocked(p *player.Player) error {
	...
	return os.WriteFile(filepath.Join(db.playerDir, fmt.Sprintf("%d.json", p.UID)), raw, 0o644)
}

func (db *DataBase) loadPlayerLocked(uid uint32) (*player.Player, error) {
	raw, err := os.ReadFile(filepath.Join(db.playerDir, fmt.Sprintf("%d.json", uid)))
	if err != nil {
		return nil, err
	}
	var p player.Player
