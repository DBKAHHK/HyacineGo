...
func (s *Server) registerSceneDependencyHandlers() {
	// Client notify used for syncing position version; 3.8.0src echoes it back.
	s.reg.Register(packet.SceneUpdatePositionVersionNotify, packet.Handler{
	...
	s.reg.Register(packet.GetFirstTalkNpcCsReq, packet.Handler{
	...
	s.reg.Register(packet.GetFirstTalkByPerformanceNpcCsReq, packet.Handler{
	...
	s.reg.Register(packet.GetNpcTakenRewardCsReq, packet.Handler{
