...
import (
	...
	"hyacine-server/internal/gameserver/trace"
...
func (s *Server) registerHandlers() {
	s.registerLoginHandlers()
	s.registerPlayerHandlers()
	s.registerAvatarHandlers()
	s.registerSceneHandlers()
	s.registerSceneDependencyHandlers()
	s.registerItemHandlers()
	s.registerPropHandlers()
	s.registerMiscHandlers()
	s.registerProfileHandlers()
	s.registerMissionHandlers()
}
...
func New(addr string, opts Options, metrics *observability.Metrics) *Server {
	...
	s.nextUID.Store(100000000) // fallback uid allocator start
...
func (s *Server) Run(ctx context.Context) error {
	...
	slog.Info("gameserver listening", "addr", s.addr, "net", "tcp")
	...
	for {
		...
		slog.Info("gameserver accepted", "remote", conn.RemoteAddr().String(), "local", conn.LocalAddr().String())
...
func (s *Server) handleConn(ctx context.Context, c net.Conn) {
	...
	sess := &session{conn: c, state: stateWaitingForToken}
...
func (s *Server) handlePacket(sess *session, cmdID uint16, payload []byte) {
	...
	s.mu.RUnlock()
	ps := &packet.Session{
		State:      mapState(sess.state),
		AccountUID: sess.accountUID,
		UID:        sess.uid,
		LastActive: time.Now(),
	}
	...
	if handled {
		sess.state = unmapState(ps.State)
		sess.accountUID = ps.AccountUID
		sess.uid = ps.UID
		return
	}
...
func mapState(s sessionState) packet.SessionState {
	switch s {
	case stateWaitingForToken:
		return packet.StateWaitingForToken
	case stateWaitingForLogin:
		return packet.StateWaitingForLogin
	default:
		return packet.StateActive
	}
}

func unmapState(s packet.SessionState) sessionState {
	switch s {
	case packet.StateWaitingForToken:
		return stateWaitingForToken
	case packet.StateWaitingForLogin:
		return stateWaitingForLogin
	default:
		return stateActive
	}
}
