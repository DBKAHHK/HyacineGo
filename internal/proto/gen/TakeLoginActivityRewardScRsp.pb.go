// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v6.33.1
// source: TakeLoginActivityRewardScRsp.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type TakeLoginActivityRewardScRsp struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Retcode       uint32                 `protobuf:"varint,7,opt,name=retcode,proto3" json:"retcode,omitempty"`
	Id            uint32                 `protobuf:"varint,8,opt,name=id,proto3" json:"id,omitempty"`
	TakeDays      uint32                 `protobuf:"varint,9,opt,name=take_days,json=takeDays,proto3" json:"take_days,omitempty"`
	PanelId       uint32                 `protobuf:"varint,10,opt,name=panel_id,json=panelId,proto3" json:"panel_id,omitempty"`
	Reward        *ItemList              `protobuf:"bytes,13,opt,name=reward,proto3" json:"reward,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TakeLoginActivityRewardScRsp) Reset() {
	*x = TakeLoginActivityRewardScRsp{}
	mi := &file_TakeLoginActivityRewardScRsp_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TakeLoginActivityRewardScRsp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TakeLoginActivityRewardScRsp) ProtoMessage() {}

func (x *TakeLoginActivityRewardScRsp) ProtoReflect() protoreflect.Message {
	mi := &file_TakeLoginActivityRewardScRsp_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TakeLoginActivityRewardScRsp.ProtoReflect.Descriptor instead.
func (*TakeLoginActivityRewardScRsp) Descriptor() ([]byte, []int) {
	return file_TakeLoginActivityRewardScRsp_proto_rawDescGZIP(), []int{0}
}

func (x *TakeLoginActivityRewardScRsp) GetRetcode() uint32 {
	if x != nil {
		return x.Retcode
	}
	return 0
}

func (x *TakeLoginActivityRewardScRsp) GetId() uint32 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *TakeLoginActivityRewardScRsp) GetTakeDays() uint32 {
	if x != nil {
		return x.TakeDays
	}
	return 0
}

func (x *TakeLoginActivityRewardScRsp) GetPanelId() uint32 {
	if x != nil {
		return x.PanelId
	}
	return 0
}

func (x *TakeLoginActivityRewardScRsp) GetReward() *ItemList {
	if x != nil {
		return x.Reward
	}
	return nil
}

var File_TakeLoginActivityRewardScRsp_proto protoreflect.FileDescriptor

const file_TakeLoginActivityRewardScRsp_proto_rawDesc = "" +
	"\n" +
	"\"TakeLoginActivityRewardScRsp.proto\x12\x13emu.lunarcore.proto\x1a\x0eItemList.proto\"\xb7\x01\n" +
	"\x1cTakeLoginActivityRewardScRsp\x12\x18\n" +
	"\aretcode\x18\a \x01(\rR\aretcode\x12\x0e\n" +
	"\x02id\x18\b \x01(\rR\x02id\x12\x1b\n" +
	"\ttake_days\x18\t \x01(\rR\btakeDays\x12\x19\n" +
	"\bpanel_id\x18\n" +
	" \x01(\rR\apanelId\x125\n" +
	"\x06reward\x18\r \x01(\v2\x1d.emu.lunarcore.proto.ItemListR\x06rewardB>\n" +
	"\x13emu.lunarcore.protoZ'hyacine-server/internal/proto/gen;protob\x06proto3"

var (
	file_TakeLoginActivityRewardScRsp_proto_rawDescOnce sync.Once
	file_TakeLoginActivityRewardScRsp_proto_rawDescData []byte
)

func file_TakeLoginActivityRewardScRsp_proto_rawDescGZIP() []byte {
	file_TakeLoginActivityRewardScRsp_proto_rawDescOnce.Do(func() {
		file_TakeLoginActivityRewardScRsp_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_TakeLoginActivityRewardScRsp_proto_rawDesc), len(file_TakeLoginActivityRewardScRsp_proto_rawDesc)))
	})
	return file_TakeLoginActivityRewardScRsp_proto_rawDescData
}

var file_TakeLoginActivityRewardScRsp_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_TakeLoginActivityRewardScRsp_proto_goTypes = []any{
	(*TakeLoginActivityRewardScRsp)(nil), // 0: emu.lunarcore.proto.TakeLoginActivityRewardScRsp
	(*ItemList)(nil),                     // 1: emu.lunarcore.proto.ItemList
}
var file_TakeLoginActivityRewardScRsp_proto_depIdxs = []int32{
	1, // 0: emu.lunarcore.proto.TakeLoginActivityRewardScRsp.reward:type_name -> emu.lunarcore.proto.ItemList
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_TakeLoginActivityRewardScRsp_proto_init() }
func file_TakeLoginActivityRewardScRsp_proto_init() {
	if File_TakeLoginActivityRewardScRsp_proto != nil {
		return
	}
	file_ItemList_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_TakeLoginActivityRewardScRsp_proto_rawDesc), len(file_TakeLoginActivityRewardScRsp_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_TakeLoginActivityRewardScRsp_proto_goTypes,
		DependencyIndexes: file_TakeLoginActivityRewardScRsp_proto_depIdxs,
		MessageInfos:      file_TakeLoginActivityRewardScRsp_proto_msgTypes,
	}.Build()
	File_TakeLoginActivityRewardScRsp_proto = out.File
	file_TakeLoginActivityRewardScRsp_proto_goTypes = nil
	file_TakeLoginActivityRewardScRsp_proto_depIdxs = nil
}
