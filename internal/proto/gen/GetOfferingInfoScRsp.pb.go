// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v6.33.1
// source: GetOfferingInfoScRsp.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type GetOfferingInfoScRsp struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	Retcode          uint32                 `protobuf:"varint,4,opt,name=retcode,proto3" json:"retcode,omitempty"`
	OfferingInfoList []*OfferingInfo        `protobuf:"bytes,14,rep,name=offering_info_list,json=offeringInfoList,proto3" json:"offering_info_list,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *GetOfferingInfoScRsp) Reset() {
	*x = GetOfferingInfoScRsp{}
	mi := &file_GetOfferingInfoScRsp_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetOfferingInfoScRsp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetOfferingInfoScRsp) ProtoMessage() {}

func (x *GetOfferingInfoScRsp) ProtoReflect() protoreflect.Message {
	mi := &file_GetOfferingInfoScRsp_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetOfferingInfoScRsp.ProtoReflect.Descriptor instead.
func (*GetOfferingInfoScRsp) Descriptor() ([]byte, []int) {
	return file_GetOfferingInfoScRsp_proto_rawDescGZIP(), []int{0}
}

func (x *GetOfferingInfoScRsp) GetRetcode() uint32 {
	if x != nil {
		return x.Retcode
	}
	return 0
}

func (x *GetOfferingInfoScRsp) GetOfferingInfoList() []*OfferingInfo {
	if x != nil {
		return x.OfferingInfoList
	}
	return nil
}

var File_GetOfferingInfoScRsp_proto protoreflect.FileDescriptor

const file_GetOfferingInfoScRsp_proto_rawDesc = "" +
	"\n" +
	"\x1aGetOfferingInfoScRsp.proto\x12\x13emu.lunarcore.proto\x1a\x12OfferingInfo.proto\"\x81\x01\n" +
	"\x14GetOfferingInfoScRsp\x12\x18\n" +
	"\aretcode\x18\x04 \x01(\rR\aretcode\x12O\n" +
	"\x12offering_info_list\x18\x0e \x03(\v2!.emu.lunarcore.proto.OfferingInfoR\x10offeringInfoListB>\n" +
	"\x13emu.lunarcore.protoZ'hyacine-server/internal/proto/gen;protob\x06proto3"

var (
	file_GetOfferingInfoScRsp_proto_rawDescOnce sync.Once
	file_GetOfferingInfoScRsp_proto_rawDescData []byte
)

func file_GetOfferingInfoScRsp_proto_rawDescGZIP() []byte {
	file_GetOfferingInfoScRsp_proto_rawDescOnce.Do(func() {
		file_GetOfferingInfoScRsp_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_GetOfferingInfoScRsp_proto_rawDesc), len(file_GetOfferingInfoScRsp_proto_rawDesc)))
	})
	return file_GetOfferingInfoScRsp_proto_rawDescData
}

var file_GetOfferingInfoScRsp_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_GetOfferingInfoScRsp_proto_goTypes = []any{
	(*GetOfferingInfoScRsp)(nil), // 0: emu.lunarcore.proto.GetOfferingInfoScRsp
	(*OfferingInfo)(nil),         // 1: emu.lunarcore.proto.OfferingInfo
}
var file_GetOfferingInfoScRsp_proto_depIdxs = []int32{
	1, // 0: emu.lunarcore.proto.GetOfferingInfoScRsp.offering_info_list:type_name -> emu.lunarcore.proto.OfferingInfo
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_GetOfferingInfoScRsp_proto_init() }
func file_GetOfferingInfoScRsp_proto_init() {
	if File_GetOfferingInfoScRsp_proto != nil {
		return
	}
	file_OfferingInfo_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_GetOfferingInfoScRsp_proto_rawDesc), len(file_GetOfferingInfoScRsp_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_GetOfferingInfoScRsp_proto_goTypes,
		DependencyIndexes: file_GetOfferingInfoScRsp_proto_depIdxs,
		MessageInfos:      file_GetOfferingInfoScRsp_proto_msgTypes,
	}.Build()
	File_GetOfferingInfoScRsp_proto = out.File
	file_GetOfferingInfoScRsp_proto_goTypes = nil
	file_GetOfferingInfoScRsp_proto_depIdxs = nil
}
