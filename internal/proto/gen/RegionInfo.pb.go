// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v6.33.1
// source: RegionInfo.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type RegionInfo struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Title         string                 `protobuf:"bytes,2,opt,name=title,proto3" json:"title,omitempty"`
	DispatchUrl   string                 `protobuf:"bytes,3,opt,name=dispatch_url,json=dispatchUrl,proto3" json:"dispatch_url,omitempty"`
	EnvType       string                 `protobuf:"bytes,4,opt,name=env_type,json=envType,proto3" json:"env_type,omitempty"`
	DisplayName   string                 `protobuf:"bytes,5,opt,name=display_name,json=displayName,proto3" json:"display_name,omitempty"`
	Msg           string                 `protobuf:"bytes,6,opt,name=msg,proto3" json:"msg,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegionInfo) Reset() {
	*x = RegionInfo{}
	mi := &file_RegionInfo_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegionInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegionInfo) ProtoMessage() {}

func (x *RegionInfo) ProtoReflect() protoreflect.Message {
	mi := &file_RegionInfo_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegionInfo.ProtoReflect.Descriptor instead.
func (*RegionInfo) Descriptor() ([]byte, []int) {
	return file_RegionInfo_proto_rawDescGZIP(), []int{0}
}

func (x *RegionInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *RegionInfo) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *RegionInfo) GetDispatchUrl() string {
	if x != nil {
		return x.DispatchUrl
	}
	return ""
}

func (x *RegionInfo) GetEnvType() string {
	if x != nil {
		return x.EnvType
	}
	return ""
}

func (x *RegionInfo) GetDisplayName() string {
	if x != nil {
		return x.DisplayName
	}
	return ""
}

func (x *RegionInfo) GetMsg() string {
	if x != nil {
		return x.Msg
	}
	return ""
}

var File_RegionInfo_proto protoreflect.FileDescriptor

const file_RegionInfo_proto_rawDesc = "" +
	"\n" +
	"\x10RegionInfo.proto\x12\x13emu.lunarcore.proto\"\xa9\x01\n" +
	"\n" +
	"RegionInfo\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x14\n" +
	"\x05title\x18\x02 \x01(\tR\x05title\x12!\n" +
	"\fdispatch_url\x18\x03 \x01(\tR\vdispatchUrl\x12\x19\n" +
	"\benv_type\x18\x04 \x01(\tR\aenvType\x12!\n" +
	"\fdisplay_name\x18\x05 \x01(\tR\vdisplayName\x12\x10\n" +
	"\x03msg\x18\x06 \x01(\tR\x03msgB>\n" +
	"\x13emu.lunarcore.protoZ'hyacine-server/internal/proto/gen;protob\x06proto3"

var (
	file_RegionInfo_proto_rawDescOnce sync.Once
	file_RegionInfo_proto_rawDescData []byte
)

func file_RegionInfo_proto_rawDescGZIP() []byte {
	file_RegionInfo_proto_rawDescOnce.Do(func() {
		file_RegionInfo_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_RegionInfo_proto_rawDesc), len(file_RegionInfo_proto_rawDesc)))
	})
	return file_RegionInfo_proto_rawDescData
}

var file_RegionInfo_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_RegionInfo_proto_goTypes = []any{
	(*RegionInfo)(nil), // 0: emu.lunarcore.proto.RegionInfo
}
var file_RegionInfo_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_RegionInfo_proto_init() }
func file_RegionInfo_proto_init() {
	if File_RegionInfo_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_RegionInfo_proto_rawDesc), len(file_RegionInfo_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_RegionInfo_proto_goTypes,
		DependencyIndexes: file_RegionInfo_proto_depIdxs,
		MessageInfos:      file_RegionInfo_proto_msgTypes,
	}.Build()
	File_RegionInfo_proto = out.File
	file_RegionInfo_proto_goTypes = nil
	file_RegionInfo_proto_depIdxs = nil
}
