...
import (
	...
	"hyacine-server/internal/gameserver/srtools"
	"hyacine-server/internal/observability"
...
func (s *Server) handleQueryGateway(w http.ResponseWriter, r *http.Request) {
	...
	dispatchSeed := r.URL.Query().Get("dispatch_seed")
	hotfix := loadHotfixForVersion(hotfixPath, version)
	if hotfix.AssetBundleURL == "UNSUPPORTED_VERSION" && strings.TrimSpace(version) != "" {
		if hf, ok := s.fetchAndCacheHotfix(hotfixPath, version, dispatchSeed); ok {
			hotfix = hf
		}
	}
	...
	msg := &pb.GateServer{
		...
		Unk1:                true,
		Unk2:                true,
		KGOGMMCNOBP:          true, // use_tcp (field 1759 in SR-CasPS protocol)
	}
	writeB64Proto(w, msg)
}

func selectHotfixHost(version string) string {
	switch {
...
func (s *Server) fetchAndCacheHotfix(hotfixPath, version, dispatchSeed string) (hotfixEntry, bool) {
	host := selectHotfixHost(version)
	if host == "" {
		return hotfixEntry{}, false
	}
	...
	q.Set("dispatch_seed", dispatchSeed)
	q.Set("language_type", "1")
	q.Set("platform_type", "2")
	q.Set("channel_id", "1")
	q.Set("sub_channel_id", "1")
	q.Set("is_need_url", "1")
	q.Set("account_type", "1")
	u.RawQuery = q.Encode()
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		req.Header.Set("User-Agent", "UnityPlayer/2021.3.21f1")
		...
		if resp.StatusCode != http.StatusOK || len(body) == 0 {
			lastErr = fmt.Errorf("status=%d body=%d", resp.StatusCode, len(body))
			time.Sleep(100 * time.Millisecond)
			continue
		}
		...
		var gs pb.GateServer
		if err := proto.Unmarshal(raw, &gs); err != nil {
			lastErr = err
			continue
		}

		hf := hotfixEntry{
			AssetBundleURL: gs.AssetBundleUrl,
			ExResourceURL:  gs.ExResourceUrl,
			LuaURL:         gs.LuaUrl,
			IfixURL:        gs.IfixUrl,
		}
		if hf.AssetBundleURL == "" || hf.ExResourceURL == "" || hf.LuaURL == "" {
			lastErr = fmt.Errorf("empty urls from upstream")
			continue
		}
		_ = putHotfixValue(hotfixPath, version, hf)
		slog.Info("hotfix fetched", "version", version, "host", host)
		return hf, true
	}
	slog.Warn("hotfix fetch failed", "version", version, "host", host, "err", lastErr)
	return hotfixEntry{}, false
}
...
func loadHotfixForVersion(path string, version string) hotfixEntry {
	...
	if err != nil {
		return unsupportedHotfix(version)
	}
	...
	if v, ok := root[version]; ok {
		if v.AssetBundleURL != "" || v.ExResourceURL != "" || v.LuaURL != "" || v.IfixURL != "" {
			return v
		}
	}
	return unsupportedHotfix(version)
...
func unsupportedHotfix(version string) hotfixEntry {
	return hotfixEntry{
		AssetBundleURL: "UNSUPPORTED_VERSION",
		ExResourceURL:  "UNSUPPORTED_VERSION",
		LuaURL:         "UNSUPPORTED_VERSION",
		IfixURL:        "",
		Msg:            fmt.Sprintf("no suitable hotfix for version: %s", version),
	}
}

func putHotfixValue(path string, version string, value hotfixEntry) error {
	version = strings.TrimSpace(version)
	if version == "" {
		return fmt.Errorf("empty version")
	}

	root := map[string]hotfixEntry{}
	if raw, err := os.ReadFile(path); err == nil {
		_ = json.Unmarshal(raw, &root)
	}
	root[version] = hotfixEntry{
		AssetBundleURL: value.AssetBundleURL,
		ExResourceURL:  value.ExResourceURL,
		LuaURL:         value.LuaURL,
		IfixURL:        value.IfixURL,
		IfixVersion:    value.IfixVersion,
		MdkResVersion:  value.MdkResVersion,
	}
